...
//    [_wkview evaluateJavaScript:@"document.documentElement.style.webkitUserSelect='none';"completionHandler:nil];swiper-slide
    
//    [_wkview evaluateJavaScript:@"document.documentElement.getElementsByClassName('swiper-slide')[0].style.display='none';"completionHandler:nil];
    [_wkview evaluateJavaScript:@"document.documentElement.getElementsByClassName('swiper-container banner_box')[0].style.display='none';"completionHandler:nil];
}
...
//    [self initTitleViewWithTitle:self.title];
...
//    configuration.allowsInlineMediaPlayback = YES;//在线播放
    ...
    _wkview = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, Screen_Width, Screen_Height-STATUS_AND_NAVIGATION_HEIGHT)]; //configuration:configuration];
    _wkview.UIDelegate = self;
    _wkview.navigationDelegate = self;
    NSMutableURLRequest *request = [NSMutableURLRequest requestWithURL:[NSURL URLWithString:@"http://app.ballinfor.com/static/worldcup/gameEvent.html"]];
...
//    _progressView.backgroundColor = [UIColor blueColor];
    ...
    _progressView.trackTintColor = FontColor;
    _progressView.progress = 0.0;
