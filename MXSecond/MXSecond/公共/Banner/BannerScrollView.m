...
- (void)updateImage {//更新图片
    for (NSUInteger i = 0; i < self.scrollView.subviews.count; i++) {//取出scrollview 上的三个子试图 imageView
        ...
        [imagV sd_setImageWithURL:[NSURL URLWithString:self.imageUrls[index]] placeholderImage:[UIImage imageNamed:@"zhanwei"] options:SDWebImageAllowInvalidSSLCertificates];//加载图片 imageUrls存的图片地址
